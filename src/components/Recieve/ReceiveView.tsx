...
function ReceiveView() {
  const [shippedData, setShippedData] = useState<ReceiveSample[]>([]);
  const [receivedData, setReceivedData] = useState<ReceiveSample[]>([]);
  const [rejectedData, setRejectedData] = useState<ReceiveSample[]>([]);
  const [activityData, setActivityData] = useState<ReceiveSample[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setShippedData(allSamples.filter((r) => r.status === "Shipped"));
      setReceivedData(allSamples.filter((r) => r.status === "Received"));
      setRejectedData(allSamples.filter((r) => r.status === "Rejected"));
      setActivityData(activitySamples);
    } catch (err) {
  ...
  const specimenOptions = useMemo(
    () => Array.from(new Set(filterSourceRows.map((r) => r.specimen_type))),
    [filterSourceRows],
  );
  const testOptions = useMemo(
    () => Array.from(new Set(filterSourceRows.map((r) => r.test_name))),
    [filterSourceRows],
  );
  const serviceOptions = useMemo(
    () => Array.from(new Set(filterSourceRows.map((r) => r.service_name))),
    [filterSourceRows],
  );
  const activityServiceOptions = useMemo(
    () => Array.from(new Set(activityData.map((r) => r.service_name))),
    [activityData],
  );
  ...
  const parseDate = (value: string) => {
    ...
    if (value.includes("/")) {
      ...
      [day, month, year] = [Number(p[0]), Number(p[1]) - 1, Number(p[2])];
    } else if (value.includes("-")) {
      const p = value.split("-");
      if (p.length !== 3) return null;
      [year, month, day] = [Number(p[0]), Number(p[1]) - 1, Number(p[2])];
    } else return null;
    if (
      !Number.isFinite(day) ||
  ...
  const filteredRows = useMemo(() => {
    ...
    return sourceRows.filter((row) => {
      ...
      if (activeTab === "activity") {
        const activityDate = parseDate(row.ship_date);
      ...
      const rowDate = parseDate(row.ship_date);
  ...
  const renderActiveTab = () => {
    if (activeTab === "received") return <ReceivedTab rows={pagedRows} />;
    if (activeTab === "rejected") return <RejectedTab rows={pagedRows} />;
    if (activeTab === "activity") return <ActivityLogsTab rows={pagedRows} />;
    return (
      <ShippedTab
        rows={pagedRows}
